...
class Qwen2_5_VLForInterCoT(Qwen2_5_VLForConditionalGeneration):
    ...
    def forward(
        ...
        if (
            ...
            # all_pos = (input_ids == self.config.image_token_id).nonzero(as_tuple=True)[1]
            # self.query_image_start = None
            # self.query_image_end = all_pos[-1] + 1
            # for i in range(1,len(all_pos)):
            #     if all_pos[-i] != all_pos[-i-1] + 1:
            #         self.query_image_start = all_pos[-i]-1
            #         break
            # if self.query_image_start is None:
            #     self.query_image_start = all_pos[0]-1
        ...
        if (
            ...
            mapping_indices = torch.nonzero(self.query_image_mask, as_tuple=True)[-1]
            image_attentions = image_attentions[self.query_image_mask]

            threshold = image_attentions.mean()
            over_threshold_mask = image_attentions > threshold
            candidate_indices = over_threshold_mask.nonzero().squeeze(-1)
            # origin = mapping_indices[candidate_indices]
            W = 14 if image_grid_thw is None else image_grid_thw[-1, -1] / 2
            coords = torch.stack([candidate_indices // W, candidate_indices % W], dim=1)
            if len(coords) > 0:
                # 使用并查集合并相邻坐标（8邻域）
                parent = torch.arange(len(coords), device=coords.device)
                ...
                for i in range(len(coords)):
                    for j in range(i + 1, len(coords)):
                        ...
                        if dx <= 1 and dy <= 1:  # 8邻域
                            union(i, j)

                # 统计连通区域大小，选择最大的
                cluster_sizes = torch.bincount(parent)
                largest_cluster_id = cluster_sizes.argmax()
                selected_indices = candidate_indices[parent == largest_cluster_id]
            else:
                # 无超过阈值的区域，回退选择top1
                selected_indices = image_attentions.topk(1)[1]
            # 计算当前区域面积和平均注意力,和之前句子的面积和平均注意力做对比
            selected_patches = selected_indices.tolist()
            selected_patches = sorted(selected_patches)
            # 计算当前区域面积和平均注意力
            current_area = len(selected_patches)
            current_attn_mean = image_attentions[selected_patches].mean()

            # 条件判断：是否满足面积更小且注意力更高
            condition_met = True  # 初始默认允许插入
            if (
                "previous_area" not in self.__dict__
                and "previous_attn_mean" not in self.__dict__
            ):
                self.previous_area = None
                self.previous_attn_mean = None
            if self.previous_area is not None and self.previous_attn_mean is not None:
                # 条件1：当前面积 < 上一句面积
                area_condition = current_area < self.previous_area
                # 条件2：当前注意力均值 > 上一句均值
                attn_condition = current_attn_mean > self.previous_attn_mean
                condition_met = area_condition & attn_condition
            for selected_patch in selected_patches:
                if selected_patch >= len(mapping_indices):
                    raise Exception()
            selected_patches = mapping_indices[selected_patches]
            ...
            if condition_met:
                ...
                # selected_patches = selected_patches.clone()#+ self.query_image_start
                ...
                self.previous_attn_mean = current_attn_mean
                # 更新掩码和位置（原有逻辑）
            ...
            # num_selected_patches_new = min(self.num_selected_patches, image_attentions.shape[0])
            #
            # selected_patches = image_attentions.topk(num_selected_patches_new)[1]
...
def calculate_generated_text(prompt, vision_x, return_image_masks=False):
    ...
    generated_text = processor.decode(out, skip_special_tokens=False)
...
def main():
    ...
    for data in tqdm(dataset):
        ...
        one_shot_vision = [
            Image.open(
                os.path.join(
                    "/mnt/hdd/zhurui/data/m3cot/images", TRAING_CASE_1["id"] + ".png"
                )
            ),
            Image.open(
                os.path.join(
