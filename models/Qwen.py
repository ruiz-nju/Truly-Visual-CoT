...
def calculate_entropy(att_map):
    ...
    # 直接取最大值，不加 axis，不加 out
    att_max = flat_att.max()
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # 这一步将 image_embeds 填充到 inputs_embeds 当中
        ...
        # added TODO 这里减少了一个 tab 的缩进，不确定正不正确
        ...
        if (
            ...
            self.new_tokens = 0  # 跟踪新生成的文本token数量
            self.num_selected_patches = 64  # TODO:转化为参数
            img_start_token_id = 151652  # <|vision_start|>
            img_end_token_id = 151653  # <|vision_end|>
            # input_ids.shape: torch.Size([1, 638])
            # 图像 embedding 起点和终点，闭区间
            ...
            self.new_tokens += 1
            # 将新生成的 token 标记为文本（即 query_image_mask 为 False）
        ...
        # self.num_line_break % 2 == 0 源代码两句话一次判断，不清楚这么设计的目的
        if pixel_values is None and output_attentions and input_ids[:, -1] == dot_id:
            # outputs.attentions[0].shape: torch.Size([1, 28, 1, 662]) 共有 28 层，每层 28 个头
            # torch.cat(outputs.attentions, dim=1).shape: torch.Size([1, 784, 1, 662])
            attentions = torch.cat(outputs.attentions, dim=1).mean(dim=1)[
                :, -1
            ]  # torch.Size([1, 662])
            ...
            if condition_met:
                print(f"满足插入图像条件, 当前断句次数: {self.num_line_break}")
                ...
                self.query_image_mask = torch.cat(
                    [
                        self.query_image_mask,  # torch.Size([1, 662])
                        torch.zeros(
                            self.query_image_mask.shape[0],  # 1
                            # self.query_vokens.shape: torch.Size([560, 3584])
                            self.query_vokens.shape[0] + 2,
                            device=self.query_image_mask.device,
                        ).bool(),  # torch.Size([1, 562])
                    ],
                    dim=1,
                )
