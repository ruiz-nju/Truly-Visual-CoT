...
IMG_FOLDER = '/mnt/hdd/zhurui/data/m3cot/images/'
EVAL_FILE = '/mnt/hdd/zhurui/data/m3cot/test.jsonl'
DATA_NAME = 'm3cot'
...
def calculate_entropy(att_map):
    ...
    att_max = torch.max(flat_att)
    exp_att = torch.exp(flat_att - att_max)
    prob_dist = exp_att / (torch.sum(exp_att) + 1e-10)
...
class Qwen2_5_VLForInterCoT(Qwen2_5_VLForConditionalGeneration):
    ...
    def forward(
        ...
        if output_attentions and (past_key_values.key_cache == [] or past_key_values.key_cache[0] == [] ) and pixel_values is not None:
            self.new_tokens = 0
            self.num_selected_patches = num_selected_patches
            img_start_token_id = 151652 #8197
            img_end_token_id = 151653
            self.query_image_start = (input_ids == img_start_token_id).nonzero(as_tuple=True)[1][-1] + 1
            self.query_image_end = (input_ids == img_end_token_id).nonzero(as_tuple=True)[1][-1]
            ...
            self.query_image_mask[:, self.query_image_start: self.query_image_end] = True # self.query_image_mask[:, self.query_image_start: self.query_image_start+1024] = True
        ...
        if pixel_values is None and output_attentions and self.num_line_break % 2 == 0 and input_ids[:, -1] == dot_id:
            ...
            # 更新历史记录
            self.past_attentions.append((self.num_line_break, image_attentions, att_map_mean, att_map_entropy))
            print(self.past_attentions[-1][0])
        ...
        if past_key_values is not None:
            if inputs_embeds is not None and input_ids.shape[1] == 0:  # Exception 4
                ...
                if 'selected_vokens' in kwargs and kwargs['selected_vokens'] is not None:
                    ...
                    if img_embeds_add is not None:
                        img_embeds_add = torch.cat([img_embeds_add, kwargs['selected_vokens']],dim=-1)
                    else:
    ...
    def _update_model_kwargs_for_generation(
            ...
            if not is_encoder_decoder:
                ...
                if "attention_mask" in model_kwargs:
                    ...
                    if 'selected_vokens' in outputs and outputs['selected_vokens'] is not None:
                        model_kwargs["attention_mask"] = torch.cat([model_kwargs["attention_mask"], torch.ones_like(outputs['selected_vokens'][:,0].unsqueeze(0)), torch.tensor([[1,1]],device=model_kwargs['attention_mask'].device, dtype=model_kwargs['attention_mask'].dtype)], dim=-1)
            else:
...
model_path = "/mnt/hdd/zhurui/models/Qwen2.5-VL-7B-Instruct"
processor = AutoProcessor.from_pretrained(model_path)
model = Qwen2_5_VLForInterCoT.from_pretrained(model_path, torch_dtype="auto",attn_implementation=config['attn']).to("cuda:0")
...
zero_shot_prompt_template = lambda question, options, image_path: [
    {
        "role": "system",
        "content": [
            {"type": "text", "text": "There is an image and a question about it. You need to have a deep thinking before answering, When you output the '.' The system will cut the most similar image for you to rethinking. Please answer the question based on the image."},
        ],
        "role": "user",
        "content": [
            {
...
def main():
    ...
    mcot_one_fh = open(f'./results/{model_name}/{DATA_NAME}/{model_name}_mcot_one.json', 'a')
    mcot_zero_fh = open(f'./results/{model_name}/{DATA_NAME}/{model_name}_mcot_zero.json', 'a')
    for data in tqdm(dataset):
        options = ''
        for i, c in zip(['A', 'B', 'C', 'D', 'E', 'F'], data['choices']):
            options += '{}. {}\n'.format(i, c)
        mcot_input_str = zero_shot_prompt_template(data['question'],options,os.path.join(IMG_FOLDER, data['id']+'.png' if DATA_NAME == 'm3cot' else data['image']))
        one_shot_vision = [Image.open(os.path.join('/mnt/hdd/zhurui/data/m3cot/images', TRAING_CASE_1['id']+'.png')),
                    Image.open(os.path.join('/mnt/hdd/zhurui/data/m3cot/images', TRAING_CASE_1['id']+'.png')),
...
def test():
    ...
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    mcot_one_fh = open(f'./results/{model_name}/{DATA_NAME}/{model_name}_mcot_one_test.json', 'a')
    mcot_zero_fh = open(f'./results/{model_name}/{DATA_NAME}/{model_name}_mcot_zero_test.json', 'a')
...
#     }
# ]
# image = Image.open('/mnt/hdd/zhurui/data/m3cot/train/images/physical-commonsense-1424.png')
